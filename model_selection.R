...
covid.lm.2a <- bas.lm(hospH8 ~ gialla + arancione + rossa + newpos +
                        intcar + hosp,
                      training.set.cat,
                      n.models = 1,
                      include.always = ~ .)
...
covid.lm.2b <- bas.lm(hospH8 ~ color.num + newpos + intcar + hosp,
                      training.set.num,
                      n.models = 1,
                      include.always = ~ .)
summary(covid.lm.2b)

test(covid.lm.2b, test.set.num, "hospH8")
...
covid.lm.3 <- bas.lm(hospH8 ~ gialla + arancione + rossa + newpos +
                       intcar + hosp + newpos_av7D,
                     training.set.cat)
summary(covid.lm.3)

test(covid.lm.3, test.set.cat, "hospH8")

# plot(covid.lm.3)

# Show which features are included in the top models selected by 'bas.lm'
plot(seq(coef(covid.lm.3)$probne0), coef(covid.lm.3)$probne0,
     type = "h",
     lwd = 4,
     xaxt = "n",
     ...
     xlab = "",
     ylab = "post p(B != 0)")
axis(1, seq(coef(covid.lm.3)$probne0), labels = coef(covid.lm.3)$namesx)
...
covid.lm.4 <- bas.lm(hospH8 ~ gialla + arancione + rossa + newpos +
                       intcar + hosp + newpos_av7D + winter + summer,
                     training.set.cat.szn)
summary(covid.lm.4)

test(covid.lm.4, test.set.cat.szn, "hospH8")

plot(seq(coef(covid.lm.4)$probne0), coef(covid.lm.4)$probne0,
     type = "h",
     lwd = 4,
     xaxt = "n",
     main = "Feature inclusion probabilities",
     xlab = "",
     ylab = "post p(B != 0)")
axis(1, seq(coef(covid.lm.4)$probne0), labels = coef(covid.lm.4)$namesx)
