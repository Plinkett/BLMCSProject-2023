...
covid.lm.2aHosp <- bas.lm(hospH8 ~ gialla + arancione + rossa + newpos +
                      ...
                      n.models = 1,
                      include.always = ~ .)
coef.lm2 <- coef(covid.lm.2aHosp)
coef.lm2
summary(covid.lm.2aHosp)

X11()
par(mfrow = c(2, 3))
plot(coef.lm2, subset = 2:7, ask = F)

test(covid.lm.2aHosp, test.set.cat, "hospH8")
...
image(covid.lm.3Intcar, rotate = F)
...
best <- which.max(covid.lm.3Intcar$logmarg)
bestmodel <- covid.lm.3Intcar$which[[best]]+1
print(bestmodel)

covid.lm.3Intcar <- bas.lm(intcarH8 ~ arancione + intcar + newpos_av7D,
                           prior="BIC",
                           bestmodel=rep(1,4),
                           n.models=1,
                           training.set.cat)
summary(covid.lm.3Intcar)
par(mfrow=c(2,2))
coef(covid.lm.3Intcar)
plot(coef(covid.lm.3Intcar))
test(covid.lm.3Intcar, test.set.cat, "intcarH8")

confint(coef(covid.lm.3Intcar))
...
for(alpha in c(0.001,0.01,0.1,1,10, 30, 40 ,50 , 60, 100, 200, 300, 450, 500, 1000)){
  covid.lm.3Hosp <- bas.lm(hospH8 ~ gialla + arancione + rossa + newpos +
                           ...
                           alpha=alpha,
                           training.set.cat)
  mse <- append(mse, test(covid.lm.3Hosp, test.set.cat, "hospH8"))
}
...
for(alpha in c(0.001,0.01,0.1,1,10, 30, 40 ,50 , 60, 100, 200, 300, 450, 500, 1000)){
  covid.lm.3Intcar <- bas.lm(intcarH8 ~ arancione + intcar + newpos_av7D,
                             ...
                             alpha=alpha,
                             training.set.cat)
  mse <- append(mse, test(covid.lm.3Intcar, test.set.cat, "hospH8"))
}
